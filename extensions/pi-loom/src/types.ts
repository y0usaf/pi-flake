...
// human.ask: the human is a callable participant in the DSL, not an approval
// channel bolted onto the side. Unlike checkpoint (boolean approve/reject) the
// answer is one of the workflow's own named choices.
export interface HumanAskInput { name: string; prompt: string; choices: readonly string[]; context: JsonValue }
// human.edit: the same participant, handed a text buffer instead of a list of
// choices. The result is a record rather than a bare string because the three
// outcomes a workflow must tell apart -- edited, left byte-identical, and
// abandoned without saving -- are not distinguishable from the text alone.
export interface HumanEditInput { name: string; prompt: string; text: string; context: JsonValue }
export interface HumanEditResult { text: string; changed: boolean; abandoned: boolean }
// human.review: the same participant handed something to judge. The verdict
// vocabulary is fixed rather than workflow-supplied (that is what human.ask is
// for) so a later stage can switch on it without knowing which review produced
// it; `note` is the reviewer's prose, and is the only channel by which a
// "changes" verdict says what to change.
export const HUMAN_REVIEW_VERDICTS = ["approve", "changes", "reject"] as const;
export type HumanReviewVerdict = (typeof HUMAN_REVIEW_VERDICTS)[number];
export interface HumanReviewInput { name: string; prompt: string; subject: string; context: JsonValue }
export interface HumanReviewResult { verdict: HumanReviewVerdict; note: string }
// dryRun: the sandbox asking the engine whether a workflow directory would be
// registered as a slash command, without launching anything. Only a directory,
// because everything else worth knowing is read off disk by the engine.
export interface DryRunInput { directory: string }
...
export interface WorkflowBridge { agent?: (prompt: string, options: Readonly<Record<string, JsonValue>>, signal: AbortSignal, identity: AgentIdentity) => Promise<JsonValue>; shell?: (command: string, options: ShellOptions, signal: AbortSignal, identity: ShellIdentity) => Promise<ShellResult>; checkpoint?: (input: Readonly<Record<string, JsonValue>>, signal: AbortSignal) => boolean | Promise<boolean>; humanAsk?: (input: Readonly<Record<string, JsonValue>>, signal: AbortSignal) => Promise<string>; humanEdit?: (input: Readonly<Record<string, JsonValue>>, signal: AbortSignal) => Promise<HumanEditResult>; humanReview?: (input: Readonly<Record<string, JsonValue>>, signal: AbortSignal) => Promise<HumanReviewResult>; dryRun?: (input: Readonly<Record<string, JsonValue>>, signal: AbortSignal) => Promise<JsonValue>; function?: (name: string, input: Readonly<Record<string, JsonValue>>, path: string, signal: AbortSignal, worktreeOwner?: string, structuralPath?: readonly string[]) => Promise<JsonValue>; worktree?: (owner: string, signal: AbortSignal) => Promise<Readonly<WorkflowWorktreeReference>>; functions?: Readonly<Record<string, { name: string }>>; phase?: (name: string) => void | Promise<void>; log?: (message: string) => void | Promise<void> }
