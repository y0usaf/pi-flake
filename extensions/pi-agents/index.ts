...
/** String enum schema compatible with Google's API and other providers that don't support anyOf/const patterns. */
...
export default function multiAgent(pi: ExtensionAPI) {
	...
	async function runWorkflow(
			...
			for (let start = 0; start < entries.length; start += chunkSize) {
				...
				for (const r of settled) {
					if (r.status === "rejected") {
						for (const other of chunk) killSubtree(other.id);
		...
		while (generation < maxGenerations) {
			...
			const doerResults = await spawnWave(tasks.map((task, i) => {
				...
				return { id, spawn: () => spawnChild(undefined, { id, system_prompt: workflow.doer.system_prompt, task, contract: workflow.doer.contract }, model, cwd, signal, onUpdate) };
			}));
			...
			const checkerResults = await spawnWave(candidates.map((c) => {
				...
				return { id, spawn: () => usePanel
					? spawnPanel(undefined, { id, system_prompt: workflow.check.system_prompt, task: buildCheckerTask(c), contract: workflow.check.contract, panel: { models: panelCfg?.models, size: panelCfg?.size } }, model, cwd, signal, onUpdate)
					: spawnChild(undefined, { id, system_prompt: workflow.check.system_prompt, task: buildCheckerTask(c), contract: workflow.check.contract }, model, cwd, signal, onUpdate) };
			}));
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			const model = ctx.model;
			if (!model) throw new Error("No model selected");
			return await runWorkflow(params.workflow as any, model, ctx.cwd, signal, onUpdate);
		},
	});
}
