 ...
 * report/submit_answers/ask_parent tools. Per DESIGN.md: "contract
 * normalization, answer validation, prompt rendering, nudge loop —
...
export function buildAskParentTool(childId: string, holder: { state?: ChildState }): AgentTool<typeof askParentSchema> {
	return {
...
export function buildSubmitAnswersTool(childId: string, contract: ContractBox): AgentTool<typeof submitAnswersSchema> {
	return {
		name: "submit_answers",
 ...
 */
export async function runUntilContractFulfilled(state: ChildState, prompt: string, signal: AbortSignal | undefined, allowAsk = true): Promise<void> {
