...
export default function darkTerminalTheme(pi: ExtensionAPI): void {
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (process.env.PI_DARK_TERMINAL === "0") return;
    ...
    const marker = join(agentDir, "dark-terminal-default-applied");
    if (canPersist && existsSync(marker)) return;

    if (!ctx.ui || typeof ctx.ui.setTheme !== "function") return;
    const result = ctx.ui.setTheme("dark-terminal");
