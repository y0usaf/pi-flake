...
- Raised the default insertion thresholds to `32768` bytes for both `APHRODITE_TOOL_THRESHOLD` and `APHRODITE_TERMINAL_THRESHOLD`. A three-model panel over 513 local session JSONL files found the previous 16 KB / 8 KB pair still lost tokens once retrieval round-trips were charged: 191 markers replaced 4,027,231 bytes, 170 retrievals recovered 142 (74%), and only the 32 KB+ bucket had a favorable retrieval/byte-win tradeoff.
