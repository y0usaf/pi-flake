...
const MORPH_ROUTING_HINT_HEADER = "Pi Morph routing:";

function buildMorphRoutingHint(settings: MorphSettings, apiKeyAvailable: boolean): string {
  if (!settings.enabled) {
    return [
...
function appendMorphRoutingHint(payload: unknown, hint: string): unknown {
