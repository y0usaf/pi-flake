...
**Where it renders.** A mid-turn questionnaire is inline: `ctx.ui.custom` installs it into pi's editor container, so it takes the editor slot and the transcript stays visible above it, exactly like pi's own selectors. The restart-resume questionnaire is the one exception and passes `{ overlay: true }`. It is opened from a `session_start` handler, and pi continues its own startup after extension handlers return — autocomplete setup, loaded-resource notices, startup dialogs — several of which call `editorContainer.clear()`. That would evict an inline component without a word, leaving the awaited promise pending forever: the notification prints, no questions appear, and the tool call stays dangling. The overlay stack is a separate layer and survives it. The flag is a parameter, not a default, because making every questionnaire a floating modal is a worse UI than the failure it prevents.
