...
export default function (pi: ExtensionAPI) {
	...
	const jsToolDefinition = {
		...
		promptGuidelines: [
			...
			"js is the only tool. Do all file work with kernel.read / kernel.edit / kernel.write, shell work with kernel.bash, and agent delegation with kernel.rlm.run inside js. rlm.run is async by default: it returns a handle immediately and the answers arrive later as an injected follow-up message — keep prompting while children run. Pass {background: false} to wait for answers in-cell.",
			...
			"Code that exceeds the timeout restarts the kernel and loses all REPL state — keep long-running work incremental. A blocking kernel.rlm.run (background:false) is bounded by its own child timeout, not the js timer; background runs are bounded by their own child timeout too.",
