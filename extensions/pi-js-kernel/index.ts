...
export default function (pi: ExtensionAPI) {
	...
	bridgeHandlers = {
		...
		"rlm.panel": rlmBridge.panel,
		"rlm.loop": rlmBridge.loop,
		"rlm.answer": rlmBridge.answer,
		"rlm.peek": rlmBridge.peek,
	};
	...
	const jsToolDefinition = {
		...
		label: "js",
		description:
			"persistent JavaScript kernel (RLM) — evaluated code keeps variables, imports, and functions across calls within the session, and can delegate file work and child agents via the kernel API. State dies with the session (a new session starts fresh). js is the single tool: all file/search/shell/agent capabilities are reached through kernel.* inside evaluated code. Available: kernel.read(path, opts) (hashline v3 LINEID-anchored file preview), kernel.edit({path, edits}) (LINEID-anchored edits), kernel.write({path, content}) (create or overwrite a file), kernel.bash(cmd, {timeoutMs}) (subshell), kernel.rlm.run(task, {contract, model, timeoutSeconds}) (spawn a child agent — async by default, returns an admission handle, answers arrive as an injected follow-up message; {background:false} blocks for in-cell answers), kernel.rlm.panel(task, {contract, models, size}) (blocking: 2-5 children on different models answer the same contract, result is an agreement tally), kernel.rlm.loop(workflow, {timeoutSeconds}) (blocking: run an agent_loop workflow — goal/doer/check/strategy/converge/budget, the same JSON as ~/.pi/workflows/), kernel.rlm.answer(id, answers) (resume a child suspended on ask_parent), kernel.rlm.peek(id) (inspect a child: status, pending questions, reports), kernel.rlm.list(), kernel.rlm.kill(id). Long-running or never-settling code hits a timeout and the kernel is restarted (state lost); an in-flight blocking kernel.rlm.run is bounded by its own child deadline, not the js timer; rlm.loop is bounded by its workflow budget or timeoutSeconds. Runs single-threaded.",
		...
		promptGuidelines: [
			...
			"js is the only tool. Do all file work with kernel.read / kernel.edit / kernel.write, shell work with kernel.bash, and agent delegation with kernel.rlm.run inside js. rlm.run is async by default: it returns a handle immediately and the answers arrive later as an injected follow-up message — keep prompting while children run. Pass {background: false} to wait for answers in-cell. kernel.rlm.panel runs 2-5 children on different models against one contract and returns an agreement tally — use it for second opinions on judgment calls; it blocks. kernel.rlm.loop runs a declarative agent_loop workflow and blocks until its budget or timeoutSeconds.",
