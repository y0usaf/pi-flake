...
- **Tool rows render as a minimalist left rail, not pi's default bg-colored Box.** Each skinned tool (`bash`, `write`, `grep`, `find`, `ls`, plus `js` via pi-js-kernel) renders with `renderShell: "self"` and a shared frame: bare `+` corner, `|` rail, content indented two, no horizontal strokes, no right rail, no background. The whole rail is indented two from the margin (`railIndent: 2`). State lives in the rail color (pending=accent, success=dim, error=error), not a box. Glyphs come from the shared symbol preset — `PI_SYMBOLS=ascii` gives the `+|+` look, unicode gives `┌│└`; the flake wrapper forces ascii. (2026-08-08)
