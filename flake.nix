{
  ...
  inputs = {
    ...
    # omp (oh-my-pi): upstream pi-mono fork that ships its own flake build
    # (bun2nix + Rust natives). We reuse its package output and wrap it with
    # the shared extension bundle instead of rebuilding it from source.
  ...
  in {
    packages = forAllSystems (system: let
      ...
      kernelPython = pkgs.python3.withPackages (ps:
        with ps;
          [
            ipykernel
            dill
    ...
    in
      ...
      // lib.optionalAttrs (oh-my-pi.packages.${system} ? omp) {
        # Reuse upstream's maintained flake build (bun2nix + Rust natives).
        omp = oh-my-pi.packages.${system}.omp;

        omp-full = self.lib.ompWithExtensions {
          inherit pkgs;
          omp = oh-my-pi.packages.${system}.omp;
          extensions = self.lib.defaultExtensionPackagesFor system;
        };
    ...
    in
      ...
      // lib.optionalAttrs (oh-my-pi.packages.${system} ? omp) {
        # omp bundle: the wrapper build succeeds and carries the extensions.
        omp-full-built = self.packages.${system}.omp-full;
      }
      // lib.optionalAttrs (system == "x86_64-linux") {
    ...
    # Per-extension install shared by the pi and omp wrappers: copy the
    # extension into <baseDir>/<name> and prepend a .pi-gate.ts shim that
    # registers all manifest entries behind PI_EXT_DISABLED. The gate
    # re-reads the env var per load, so sessions can switch a bundled
    # extension off for their lifetime.
    lib.mkExtensionGateInstall = {
      pkgs,
      extensions,
      baseDir,
      themesDir ? null,
      ...
      pkgs.lib.concatStringsSep "\n" (pkgs.lib.mapAttrsToList (name: ext: ''
          ext_dir="${baseDir}/${name}"
          mkdir -p "$ext_dir"
          cp -R ${ext}/* "$ext_dir/" 2>/dev/null || true
          ${pkgs.lib.optionalString (themesDir != null) ''
            if [ -d "$ext_dir/themes" ]; then
              mkdir -p "${themesDir}"
              cp "$ext_dir"/themes/*.json "${themesDir}"
            fi
          ''}

          entries=$(${pkgs.jq}/bin/jq -r '.pi.extensions // [] | .[]' "$ext_dir/package.json" 2>/dev/null || true)
          if [ -z "$entries" ]; then
            # No pi.extensions manifest: the host falls back to index.ts/index.js.
    ...
    in
      pkgs.stdenvNoCC.mkDerivation {
        ...
        installPhase = ''
                  ...
                  # Per-extension copy + .pi-gate.ts shim (shared helper).
                  ${self.lib.mkExtensionGateInstall {
            inherit pkgs extensions;
            baseDir = "$out/share/pi/extensions";
            themesDir = "$out/share/pi/themes";
          }}
    ...
    # Library function to build omp (oh-my-pi) with the shared extension
    # bundle. omp has no PI_DEFAULT_PACKAGES equivalent, so bundled
    # extensions are injected through a store config overlay passed via
    # PI_CONFIG_FILES (loads before --config overlays). The overlay's
    # `extensions:` array REPLACES any extensions list from global/project
    # config: for this build the bundle is authoritative.
    lib.ompWithExtensions = {
      pkgs,
      omp,
      extensions,
    }: let
    ...
    in
      pkgs.stdenvNoCC.mkDerivation {
        ...
        installPhase = ''
                  mkdir -p $out/bin $out/share/omp/extensions

                  # Symlink the omp binary
                  ln -s ${omp}/bin/omp $out/bin/.omp-real

                  # Per-extension copy + .pi-gate.ts shim (shared helper).
                  ${self.lib.mkExtensionGateInstall {
            inherit pkgs extensions;
            baseDir = "$out/share/omp/extensions";
          }}

                  # Config overlay listing the bundled extension dirs.
                  # Absolute store paths; omp resolves them through the same
                  # package.json manifest path as `-e <dir>`.
                  {
                    echo "extensions:"
                    for name in ${pkgs.lib.concatStringsSep " " sortedNames}; do
                      echo "  - $out/share/omp/extensions/$name"
                    done
                  } > $out/share/omp/bundle.yml

                  # Wrapper that exposes bundled extensions without mutating settings.
                  cat > $out/bin/.omp-wrapped << 'wrapper'
          #!/bin/bash
          set -euo pipefail

          # Prepend the bundle overlay: later PI_CONFIG_FILES entries and
          # --config overlays load after it and may replace its arrays.
          export PI_CONFIG_FILES="@out@/share/omp/bundle.yml''${PI_CONFIG_FILES:+:$PI_CONFIG_FILES}"

          # Run omp
          exec "@out@/bin/.omp-real" "$@"
          wrapper

                  substituteInPlace $out/bin/.omp-wrapped \
                    --replace-fail '@out@' "$out"

                  # Make wrapper executable
                  chmod +x $out/bin/.omp-wrapped
                  mv $out/bin/.omp-wrapped $out/bin/omp
        '';
